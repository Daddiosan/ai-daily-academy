...
def newest_lesson() -> Path:
    candidates = sorted(LESSONS_DIR.glob("day_*.txt"))
    if not candidates:
        raise RuntimeError("No lesson txt found.")
    return candidates[-1]
...
def is_heading(line: str) -> bool:
    ...
    if re.match(r"^\d+[\.．、]\s*", original):
        return True
    if re.match(r"^(オープニング|前回の復習|今日の|まとめ|要点|理解チェック|次回予告|最新AI)", original):
        return True
    return False
...
def make_pdf(lesson_path: Path) -> Path:
    ...
    doc = SimpleDocTemplate(
        ...
        topMargin=18 * mm,
        bottomMargin=18 * mm,
        title="AI Daily Academy",
        author="AI Daily Academy",
    )
    ...
    for raw in lines:
        ...
        # Markdown URLを、PDFではクリック可能リンクにする
...
def main():
    ...
    lesson_path = Path(args.lesson) if args.lesson else newest_lesson()
